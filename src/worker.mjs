...
const DEFAULT_IMAGE_MODEL = "gemini-1.5-flash-preview-image-generation";
...
async function handleImageGeneration(req, apiKey) {
  ...
  const { prompt, n: requestedN = 1, size = "1024x1024" } = req; // size is noted but not used yet for Gemini

  if (!prompt || typeof prompt !== 'string' || prompt.trim() === "") {
    throw new HttpError("Missing or invalid 'prompt' in request body", 400);
  }

  const n = Math.max(1, Math.min(parseInt(requestedN, 10) || 1, 4)); // Request 1 to 4 images, default 1. (Adjust max if known for Gemini)
  ...
  const geminiPayload = {
    ...
    generationConfig: {
      response_modalities: ['TEXT', 'IMAGE'], // Based on user's Python example
      candidateCount: n
    },
    safetySettings, // Defined globally in the file
  };
  ...
  if (imageData.length === 0) {
    ...
    if (geminiResponse.promptFeedback?.blockReason) {
        // Log safety ratings if available for more details
        ...
        throw new HttpError(`Prompt blocked by Gemini for reason: ${geminiResponse.promptFeedback.blockReason}`, 400);
    }
  ...
  const openAiResponse = {
    ...
    data: imageData.slice(0, n), // Return up to n images, respecting original request
  };
